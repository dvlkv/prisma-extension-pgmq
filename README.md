...
A TypeScript library that provides a Prisma plugin for PostgreSQL Message Queue (PGMQ), enabling type-safe message queue operations in your Prisma-based applications.
...
// Read messages
await prisma.$transaction(async (tx) => {
  const messages = await pgmq.read(tx, 'my-work-queue', 30, 5);
  ...
  for (const message of messages) {
    ...
    await pgmq.deleteMessage(tx, 'my-work-queue', message.msg_id);
  }
});
```
...
The class-based API provides automatic transaction management and a cleaner interface:
...
// Create a queue
await pgmq.createQueue('my-work-queue');

// Send a message
const msgId = await pgmq.send('my-work-queue', {
  userId: 123,
  action: 'send-email',
  email: 'user@example.com'
});
...
const messages = await pgmq.read('my-work-queue', 30, 5);
...
for (const message of messages) {
  ...
  // Delete the message when done
  await pgmq.deleteMessage('my-work-queue', message.msg_id);
}
```
...
```typescript
const result = await pgmq.transaction(async (tx) => {
  // Send multiple related messages
  const msgId1 = await tx.send('queue1', { type: 'user-signup', userId: 123 });
  const msgId2 = await tx.send('queue2', { type: 'send-welcome-email', userId: 123 });
  
  // Update queue metrics
  const metrics = await tx.metrics('queue1');
...
```typescript
// Send immediately
const msgId = await pgmq.send('my-queue', { data: 'hello' });

// Send with delay (seconds)
const msgId = await pgmq.send('my-queue', { data: 'hello' }, 30);

// Send with specific time
const msgId = await pgmq.send('my-queue', { data: 'hello' }, new Date('2024-01-01T10:00:00Z'));
```
...
```typescript
const msgIds = await pgmq.sendBatch('my-queue', [
  { id: 1, data: 'message 1' },
  { id: 2, data: 'message 2' },
  { id: 3, data: 'message 3' }
]);
...
const messages = await pgmq.read('my-queue', 30, 5);

// Read with conditional filtering
const messages = await pgmq.read('my-queue', 30, 5, { priority: 'high' });
```
...
const messages = await pgmq.readWithPoll('my-queue', 30, 1, 10, 500);
```
...
```typescript
const messages = await pgmq.pop('my-queue');
```
...
```typescript
const deleted = await pgmq.deleteMessage('my-queue', 123);
```
...
```typescript
const deletedIds = await pgmq.deleteBatch('my-queue', [123, 124, 125]);
```
...
```typescript
const archived = await pgmq.archive('my-queue', 123);
```
...
```typescript
const archivedIds = await pgmq.archiveBatch('my-queue', [123, 124, 125]);
```
...
```typescript
await pgmq.createQueue('my-new-queue');
```
...
```typescript
await pgmq.createPartitionedQueue('high-volume-queue', '10000', '100000');
```
...
```typescript
await pgmq.createUnloggedQueue('temp-queue');
```
...
```typescript
const dropped = await pgmq.dropQueue('old-queue');
```
...
```typescript
const messageCount = await pgmq.purgeQueue('my-queue');
```
...
```typescript
const message = await pgmq.setVt('my-queue', 123, 60); // 60 seconds
```
...
```typescript
const queues = await pgmq.listQueues();
...
```typescript
const metrics = await pgmq.metrics('my-queue');
console.log(metrics);
// {
//   queue_name: 'my-queue',
//   queue_length: 5n,
...
```typescript
const allMetrics = await pgmq.metricsAll();
```
...
const messages = await pgmq.read('quick-tasks', 30);

// For longer operations (5 minutes)
const messages = await pgmq.read('heavy-tasks', 300);
```
...
```typescript
await pgmq.transaction(async (tx) => {
  // Send notification message
  const notificationId = await tx.send('notifications', {
    type: 'order-confirmation',
    orderId: order.id
  });
  
  // Send processing message
  const processingId = await tx.send('order-processing', {
    orderId: order.id,
    notificationId
  });
...
async function processMessages() {
  while (true) {
    const messages = await pgmq.readWithPoll('work-queue', 30, 5, 10, 1000);
    ...
    for (const message of messages) {
      try {
        ...
        // Delete on success
        await pgmq.deleteMessage('work-queue', message.msg_id);
      } catch (error) {
        console.error('Task failed:', error);
        // Archive failed messages for later analysis
        await pgmq.archive('work-queue', message.msg_id);
      }
    }
  }
}
...
await pgmq.send('scheduled-tasks', {
  type: 'send-reminder',
  userId: 123,
...
const highPriorityMessages = await pgmq.read('tasks', 30, 10, { priority: 'high' });

if (highPriorityMessages.length === 0) {
  // Fall back to normal priority
  const normalMessages = await pgmq.read('tasks', 30, 10, { priority: 'normal' });
}
```
